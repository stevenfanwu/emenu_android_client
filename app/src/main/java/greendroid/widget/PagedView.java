...
public class PagedView extends ViewGroup {
    ...
    private void initPagedView() {
        ...
        final DisplayMetrics metrics = context.getResources()
                .getDisplayMetrics();
        mMinimumVelocity = (int) (metrics.density
                * MINIMUM_PAGE_CHANGE_VELOCITY + 0.5f);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_DOWN:
             ...
             * drag; otherwise don't. mScroller.isFinished should be false when
             * being flinged.
             */
...
//        Log.i(TAG, "mOffsetX:" + mOffsetX + ",mStartOffsetX:" + mStartOffsetX
//                + ",mStartMotionX:" + mStartMotionX + ",mCurrentPage:"
//                + mCurrentPage);
        final int action = ev.getAction();
        final int x = (int) ev.getX();
    ...
    private static boolean GD_WARNING_LOGS_ENABLED = false;
    ...
    private View obtainView(int position) {
        ...
        if (child == null) {
            throw new NullPointerException(
                    "PagedAdapter.getView must return a non-null View");
        }
        if (recycled != null && child != recycled) {
            if (GD_WARNING_LOGS_ENABLED) {
                Log.w(LOG_TAG,
                        "Not reusing the convertView may impact PagedView performance.");
            }
        }
        ...
        if (mCurrentPage != newPage) {
            if (mOnPageChangeListener != null) {
                mOnPageChangeListener
                        .onPageChanged(this, mCurrentPage, newPage);
            }
